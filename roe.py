...
def get_stock_roe(ticker: str) -> str:
    """
    Scrapes Finviz to retrieve the ROE for the given stock ticker.
    """
    ...
    for row in rows:
        ...
        for i in range(0, len(cells), 2):
            ...
            if key == "ROE":
                return value
    print(f"ROE not found for ticker: {ticker}")
    return None
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if roe:
            print(f"Return on Equity for {ticker} is: {roe}\n")
        else:
            print("Unable to retrieve ROE. Please check the ticker and try again.\n")