...
def get_debt_to_equity(ticker: str) -> str:
    """
    Scrapes Finviz to retrieve the Debt to Equity ratio for the given stock ticker.
    """
    ...
    for row in rows:
        ...
        for i in range(0, len(cells), 2):
            ...
            if key == "Debt/Eq":
                return value
    print(f"Debt to Equity ratio not found for ticker: {ticker}")
    return None
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if ticker.lower() in ['exit', 'quit']:
            print("Exiting...")
            break
        ratio = get_debt_to_equity(ticker)
        if ratio:
            print(f"Debt to Equity ratio for {ticker} is: {ratio}\n")
        else:
            print("Unable to retrieve Debt to Equity ratio. Please check the ticker and try again.\n")